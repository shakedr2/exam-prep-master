...
print("a","b","c",sep="-")$v$,'a-b-c','','','','a',$v$sep מחליף את המפריד בין ארגומנטים מרווח למקף.$v$,$v$לחשוב ש-sep מתווסף אחרי כל ערך.$v$,true),
...
n = ___(input("מספר: "))$v$,'int','','','','a',$v$int() ממיר את המחרוזת שמחזירה input() למספר שלם.$v$,$v$לשכוח את ההמרה ולקבל שגיאה בפעולה מתמטית.$v$,true),
...
y = x
x = 10
print(x, y)$v$,'10 5','','','','a',$v$השמה מעתיקה ערך. y קיבל 5 לפני שינוי x.$v$,$v$לחשוב ש-y משתנה יחד עם x.$v$,true);
...
print(2 + 3 * 4 - 1)$v$,'13','','','','a',$v$כפל לפני חיבור: 3*4=12, 2+12-1=13.$v$,$v$לחשב משמאל לימין בלי כללי קדימות.$v$,true),
...
result = ___(x)$v$,'abs','','','','a',$v$abs() היא פונקציה מובנית שמחזירה ערך מוחלט.$v$,$v$לכתוב פונקציה ידנית עם if.$v$,true),
...
print((10 - 2) ** 2 // 3)$v$,'21','','','','a',$v$סוגריים: 10-2=8. חזקה: 8**2=64. חלוקה שלמה: 64//3=21.$v$,$v$לשכוח סדר קדימות.$v$,true);
...
if x >= 90:
    ...
    print("נכשל")$v$,'טוב','','','','a',$v$הענף הראשון שמתקיים הוא x>=70, ושאר ה-elif לא נבדקים.$v$,$v$להמשיך לבדוק elif אחרי שתנאי התקיים.$v$,true),
...
if ___:
    print("בטווח")$v$,$v$1 <= x <= 10$v$,'','','','a',$v$בפייתון ניתן לשרשר השוואות: 1<=x<=10 שקול ל-(1<=x) and (x<=10).$v$,$v$לכתוב 1<=x and x<=10 (עובד אך ארוך).$v$,true),
...
if a > 3 and (b < 5 or b > 8):
    ...
    print("no")$v$,'yes','','','','a',$v$a>3 True. b<5 False, b>8 True, or = True. True and True = True.$v$,$v$להחליט לפי תנאי יחיד בלי לסיים צרופים.$v$,true);
...
for i in range(1, 5):
    s += i
print(s)$v$,'10','','','','a',$v$range(1,5) מפיק 1..4. 1+2+3+4=10.$v$,$v$לכלול את 5 ולקבל 15.$v$,true),
...
for i in ___:
    print(i)$v$,$v$range(0, 11, 2)$v$,'','','','a',$v$range(start, stop, step): 0 עד 11 (לא כולל) בצעדים של 2.$v$,$v$להשתמש ב-range(10) ולפספס את 10.$v$,true),
...
for i in range(3):
    for j in range(i + 1):
        total += 1
print(total)$v$,'6','','','','a',$v$i=0 → פעם, i=1 → פעמיים, i=2 → שלוש. 1+2+3=6.$v$,$v$לחשוב שהלולאה הפנימית קבועה.$v$,true);
...
def square(x):
    return x * x
print(square(4) + square(3))$v$,'25','','','','a',$v$square(4)=16, square(3)=9, סכום 25.$v$,$v$לחבר 4+3 במקום להעלות בריבוע.$v$,true),
...
def add(a, b):
    ___ a + b$v$,'return','','','','a',$v$המילה return מחזירה ערך מפונקציה.$v$,$v$לכתוב print במקום return.$v$,true),
...
def f(n):
    if n == 0:
        return 0
    return n + f(n - 1)
print(f(5))$v$,'15','','','','a',$v$5+4+3+2+1+0=15.$v$,$v$לשכוח את f(0)=0.$v$,true);
...
print(s.upper())
print(s)$v$,$v$HELLO
hello$v$,'','','','a',$v$upper מחזירה מחרוזת חדשה. s המקורי נשאר.$v$,$v$לחשוב ש-upper משנה במקום.$v$,true),
...
pos = "hello".___("o")$v$,'find','','','','a',$v$find() מחזירה את האינדקס הראשון, או -1 אם לא נמצא.$v$,$v$להשתמש ב-index ולא לטפל בחריגה.$v$,true),
...
s = "abcdef"
print(s[::-1])$v$,'fedcba','','','','a',$v$חיתוך עם step -1 הופך את המחרוזת.$v$,$v$לחשוב שצריך לולאה ידנית.$v$,true);
...
lst = [3, 1, 2]
lst.sort()
print(lst)$v$,$v$[1, 2, 3]$v$,'','','','a',$v$sort ממיינת במקום בלי להחזיר ערך.$v$,$v$לצפות ש-lst.sort() יחזיר ערך.$v$,true),
...
n = ___(lst)$v$,'len','','','','a',$v$len() מחזירה את מספר האיברים.$v$,$v$לכתוב lst.length() כמו בשפות אחרות.$v$,true),
...
for row in m:
    total += row[0]
print(total)$v$,'9','','','','a',$v$מסכמים את האיבר הראשון בכל שורה: 1+3+5=9.$v$,$v$לסכם את כל האיברים.$v$,true);
...
for k in d:
    total += d[k]
print(total)$v$,'30','','','','a',$v$איטרציה על מילון נותנת מפתחות. הסכום 10+20=30.$v$,$v$לחשוב שהלולאה רצה על ערכים ולסכם מפתחות.$v$,true),
...
___
print(d)$v$,$v$d["c"] = 3$v$,'','','','a',$v$הוספת ערך למילון נעשית בהשמה למפתח חדש.$v$,$v$לכתוב d.add("c", 3) — לא קיים במילונים.$v$,true),
...
for ch in text:
    d[ch] = d.get(ch, 0) + 1
print(d)$v$,$v${'a': 2, 'b': 2}$v$,'','','','a',$v$d.get(ch, 0) מחזיר את הערך הקיים או 0. a ו-b מופיעים פעמיים כל אחד.$v$,$v$לקבל KeyError מגישה ישירה.$v$,true);
