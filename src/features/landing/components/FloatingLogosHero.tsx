...
/**
 * Parallaxed floating logo backdrop for the landing hero.
 *
 ...
 */
export function FloatingLogosHero({ containerRef }: { containerRef: React.RefObject<HTMLElement> }) {
  ...
  useEffect(() => {
    ...
    const tiles = Array.from(stage.querySelectorAll<HTMLDivElement>("[data-floater]"));
    ...
    const tick = (t: number) => {
      const time = t * 0.001;
      const scrollProg = Math.min(1, scrollY / container.offsetHeight);
      tiles.forEach((el, i) => {
        const cfg = FLOATER_CONFIG[i];
    ...
    >
      {FLOATER_CONFIG.map((cfg, i) => (
        <div
          key={i}
          data-floater
          className="lf-floater absolute will-change-transform"
          style={{
            left: `${cfg.x}%`,
            top: `${cfg.y}%`,
            width: cfg.size,
